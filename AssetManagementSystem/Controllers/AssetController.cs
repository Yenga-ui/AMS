...
public class AssetController : ControllerBase
{
    ...
    {
        _context = context;
        _emailService = emailService; // This was missing
    }
    ...
    public async Task<IActionResult> AssignAsset(AssignAssetDto assignDto)
    {
        ...
        try
        {
            ...
            {
                var emailBody = EmailTemplateService.GetAssignmentEmailTemplate(
                  employee.Fullname,
                  (DateTime)assignment.CreatedDate,
                  asset.Make,
                  asset.Model,
                  asset.SerialNo,
                  employee.JobTitle,
                ...
                await _emailService.SendEmailAsync(employee.Email, "Asset Assignment Notification", emailBody,"");
            }
    ...
    public async Task<IActionResult> BorrowAsset(BorrowAssetDto borrowDto)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(employee.Email))
            {
                var emailBody = EmailTemplateService.GetBorrowEmailTemplate(
       employee.Fullname,
       (DateTime)borrow.CreatedDate,
       asset.Category?.Category ?? "Not specified",
       asset.Make,
       asset.Model,
       asset.SerialNo,
       employee.JobTitle,
       (DateTime)borrow.DateFrom,
       (DateTime)borrow.DateTo
   );
                await _emailService.SendEmailAsync(employee.Email, "Asset Borrow Notification", emailBody,"AMS");
            }
    ...
    public async Task<IActionResult> MaintainAsset(MaintainAssetDto maintainDto)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(employee.Email))
            {
                var emailBody = EmailTemplateService.GetMaintenanceEmailTemplate(
               employee.Fullname,
               (DateTime)maintenance.CheckInDate,
               asset.Category?.Category ?? "Not specified",
               asset.Make,
               asset.Model,
               asset.SerialNo,
               ...
               (DateTime)maintenance.ReturnDate
           );
                await _emailService.SendEmailAsync(employee.Email, "Asset Maintenance Notification", emailBody, "AMS");
            }
    ...
    public async Task<IActionResult> ReturnAsset(ReturnAssetDto returnDto)
    {
        ...
        try
        {
            ...
            var assignment = await _context.AppAssignments.FirstOrDefaultAsync(a => a.StockId == returnDto.StockId && a.Status == 1);
            var borrow = await _context.AppBorrows.FirstOrDefaultAsync(b => b.StockId == returnDto.StockId && b.Status == 1);

            if (assignment == null && borrow == null) return BadRequest("Asset is not currently assigned or borrowed");

            if (assignment != null) { assignment.Status = 0; assignment.LastUpdateDate = DateTime.Now; }
            if (borrow != null) { borrow.Status = 0; }
